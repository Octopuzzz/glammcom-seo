...
export default function AdminAboutPage() {
  ...
  useEffect(() => {
    if (!token) return;
    cmsClient.search<AboutContent>('cp_about', {}, token).then((items) => {
      if (items[0]) { setItem(items[0]); setForm(items[0]); }
    });
  }, [token]);

  const set = (field: keyof AboutContent, value: unknown) =>
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (item?.id) {
        await cmsClient.update('cp_about', item.id, form, token);
      } else {
        await cmsClient.create('cp_about', form, token);
      }
      setSaved(true);
  ...
  const fields: { key: keyof AboutContent; label: string; type?: string; textarea?: boolean }[] = [
    ...
    { key: 'artworks_count', label: 'Artworks Count', type: 'number' },
    { key: 'exhibitions_count', label: 'Exhibitions Count', type: 'number' },
    { key: 'bio', label: 'Bio', textarea: true },
    { key: 'story', label: 'Artist Story (shown as quote)', textarea: true },
  ];
    ...
    <div className="min-h-screen px-6 py-12">
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h1 className="font-[family-name:var(--font-playfair)] text-2xl font-bold text-stone-100">About</h1>
        </div>
        ...
        <div className="glass-card rounded-3xl p-8">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={saving}
              className="w-full py-3.5 bg-amber-300 hover:bg-amber-200 disabled:opacity-50 text-stone-950 font-semibold text-sm tracking-wide rounded-xl transition-all duration-300 font-[family-name:var(--font-inter)] flex items-center justify-center gap-2"
            >
              {saved ? (
                <><Check size={16} /> Saved!</>
